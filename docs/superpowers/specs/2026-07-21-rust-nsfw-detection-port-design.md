...
| ClickHouse | `clickhouse` crate (clickhouse.rs) | async-native HTTP client — fixes a real Python issue where the sync `clickhouse-connect` client blocks the event loop in the flush-worker hot path |
...
- Unauthenticated: `GET /health` (always `200 {"status":"ok"}`), `GET /ready` (checks postgres/kvrocks/clickhouse/gpu-configured/ffmpeg/ffprobe presence; `200` if all ready, `503` otherwise), `GET /openapi.json`, `GET /docs` (Swagger UI — inherits Python's current lack of auth on its docs endpoint; flagged, not newly introduced).
...
2. **Per-job task** (spawned, bounded by `worker_max_concurrent_jobs` semaphore, §6.1):
   ...
   - Download: `reqwest` streamed GET, enforce `video_max_bytes` (default 512 MiB) and `video_download_timeout_seconds` (default 120s), redact the URL from any logged error / Sentry event, **single attempt** — no retry at this layer (retry happens at the whole-job level via queue retry, matching Python).
   ...
   - `aggregate()` (pure function in `core`, §7.1 logic exactly): `final_is_nsfw = any frame is_nsfw`, `final_score = max_overall_severity / 5.0`, `move_required = final_score >= move_threshold` (default 0.8), top-category tie-break via `RISK_ORDER`. Empty frame list is a terminal error, not a panic.
...
Three existing tables ported verbatim: `yral.video_nsfw_detection` (45 columns), `yral.video_nsfw_agg` (old-compatible schema, `gcs_video_id` nullable), `yral.video_nsfw_storage_actions`. DDL is in the source repo's `db/clickhouse/*.sql` and confirmed matching `DESCRIBE TABLE` output in the audit — port verbatim, `ReplacingMergeTree` engine, same partition/order keys.
...
3. **Data layer** — Postgres migrations (§13.1), ClickHouse DDL incl. new `excluded_videos` (§13.2), KVRocks key scheme, repository traits + real impls + in-memory fakes (§13.4).
